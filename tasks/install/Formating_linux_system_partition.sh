logSecure "Initializing LUKS encryption on ${DISK}2"
...
logCreate "Creating physical volume on /dev/mapper/cryptroot"
pvcreate /dev/mapper/cryptroot
logSuccess "Physical volume created on /dev/mapper/cryptroot"

logCreate "Creating volume group 'vg0'"
...
logCreate "Creating logical volume 'swap' with size ${HIBERNATION_SWAP_SIZE}..."
lvcreate -L "$HIBERNATION_SWAP_SIZE" vg0 -n swap
logSuccess "Logical volume 'swap' (${HIBERNATION_SWAP_SIZE}) created successfully."

logCreate "Creating logical volume 'root' (remaining space)"
